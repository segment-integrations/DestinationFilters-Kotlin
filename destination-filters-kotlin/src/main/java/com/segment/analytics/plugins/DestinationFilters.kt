...
class DestinationFilters : Plugin {
    ...
    private fun removeExistingFilters() {
        for (dest in analytics.findAll(DestinationPlugin::class)) {
            for (filter in dest.findAll(DestinationFilter::class)) {
                dest.remove(filter)
            }
        }
    }

    private fun createFilter(destination: String, rule: JsonObject): Boolean {
        analytics.find(destination)?.let { dest ->
            val filter = DestinationFilter(WeakReference(engine), rule)
            dest.add(filter)
            return true
        }
        return false
...
internal class DestinationFilter(
    val engineRef: WeakReference<JSScope?>,
    val rule: JsonObject
): Plugin {
    override lateinit var analytics: Analytics
